...
export default class Matrix<T = any> {
    @observable.shallow grid: Grid<T>;
    ...
    static landingGridPosition(haystack: Grid<>, needle: Grid<>, position: Position): Position {
        ...
        while (this.gridAvailable(haystack, needle, {x: position.x, y: position.y + y + 1})) {
            y++;

            if (y >= 20) {
                console.log('something is really messed up', position.y, y);
                break;
            }
        }

        return {x: position.x, y: position.y + y};
    }
    ...
    static eachValue(grid: Grid<>, callback: Function): void {
        return this.mapValue(grid, callback);
    }
